...
const form = useWriteOffFormStore();
...
const saveAndClose = async () => {
  ...
  try {
    ...
    notify.notify('Акт списания проведен успешно', 'success');
    emit('refresh');
    emit('close');
  } catch (e) {
...
<template>
  <v-card class="h-100 d-flex flex-column">
    <v-toolbar density="compact" color="blue-grey-lighten-5">
      ...
      <v-btn color="error" variant="text" @click="onDeleteClick" v-if="form.id">Удалить</v-btn>
    ...
    <v-card-text class="flex-grow-1 overflow-y-auto pt-4">
      ...
      <div class="d-flex align-center mt-4 mb-2">
        <h3 class="text-h6">Списываемые товары</h3>
    ...
    <v-dialog v-model="deleteDialog" max-width="400">
      <v-card title="Удаление" text="Вы уверены, что хотите удалить этот акт списания?" >
